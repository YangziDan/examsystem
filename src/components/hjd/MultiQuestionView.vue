...
async function getQuestion(questionId) {
  const response = await axios.get("http://localhost:8999/multiQuestion/getMultiQuestion",{params:{questionId}})
  question.value=response.data
}

function handleClick(){
  isShowResult.value=true;
  if (choose.value==question.value.rightAnswer){
    ok();
    setStatus();
  }else{
    err();
    ...
    <div class="quesion-view">
      ...
      <div class="answer">
        <el-radio-group v-model="choose">
          <el-radio label="A">A. {{question.answerA}}</el-radio>
          <el-radio label="B">B. {{question.answerB}}</el-radio>
          <el-radio label="C">C. {{question.answerC}}</el-radio>
          <el-radio label="D">D. {{question.answerD}}</el-radio>
        </el-radio-group>
      </div>
    </div>
