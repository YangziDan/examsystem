...
async function getQuestion(questionId) {
  const response = await axios.get("http://localhost:8999/fillQuestion/getFillQuestion",{params:{questionId}})
  ...
  <el-card class="box-card">
    <div class="quesion-view">
      <div class="description">
        <div style="margin-bottom: 10px">填空题:</div>
        <div>{{question.question}}</div>
      </div>

      <div class="answer">
        <span>输入您的答案:</span>
        <el-input class="input" v-model="answer"></el-input>
      </div>
    </div>
